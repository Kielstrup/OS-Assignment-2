...
/**
 * @name    Test non-first-fit behavior in allocation
 * @brief   Ensures allocation behavior doesn't follow the first-fit principle in a location other than the first free block
 */

START_TEST (test_non_first_fit_allocation)
{
   // Allocation of initial blocks
   void * a = MALLOC(0x200); // Block A (512 bytes)
   void * b = MALLOC(0x100); // Block B (256 bytes)
   void * c = MALLOC(0x300); // Block C (768 bytes)

   // Ensure initial allocations aren't null
  ...
  // Ensure allocation of D was successful
  ck_assert_ptr_nonnull(d);

  // Ensure that D wasn't placed in the space of A (not in the first free block)
  ck_assert(d != a);

  // Ensure D was allocated within C
  ck_assert(d >= c && d < (void *)((uintptr_t)c + 0x300));
...
Suite* simple_malloc_suite()
{
  ...
  tcase_add_test (tc_core, test_non_first_fit_allocation);
